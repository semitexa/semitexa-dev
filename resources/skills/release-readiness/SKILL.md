...
- the phpstan gate checks NEUTRALITY, not cleanliness: the project sits above its own baseline, so the bar is that a release does not raise the count. `packages/semitexa-dev/resources/phpstan/phpstan-ceiling.json` records it (no environment override). Above the ceiling fails; below it warns with the exact edit (the clone counts differently from the workspace) — lower `ceiling` to lock an improvement in; raise it only deliberately, with a `deliberate` entry saying what grew
