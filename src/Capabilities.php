...
#[Capability(
    id: 'dev.observatory',
    summary: 'Live observation of every process (requests, SSE, scheduler runs, queue messages) with deep-dive traces and sandbox replay: the /__observatory panel for humans, ai:observe ps|tail|show|replay for agents.',
    useWhen: 'Debugging a Semitexa app: what is running right now, what just happened, where the milliseconds or the N+1 went, or re-running a recorded request with mutated inputs — writes rolled back, queue handoffs captured.',
    avoidWhen: 'Production (APP_ENV gates everything to dev) or profiling for microbenchmarks — spans are semantic framework steps, not a sampling profiler.',
    replaces: [
