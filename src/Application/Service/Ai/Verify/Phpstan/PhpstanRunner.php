...
final class PhpstanRunner
{
    ...
    private function suggestionFor(string $identifier): string
    {
        return match ($identifier) {
            ...
            'semitexa.builtSqlFragment'          => 'Write the whereRaw() fragment as a literal. The bindings are bound but the fragment is concatenated into the statement verbatim, so every VALUE belongs in a ? placeholder and every identifier that is not written out by hand belongs in Semitexa\\Orm\\Adapter\\SqlIdentifier::quote(). A fragment assembled with sprintf(), implode() or concatenation is not safe by construction even when it happens to be safe today; if this call composes it from identifiers that already went through SqlIdentifier, record it in AcceptedViolations with that reason.',
