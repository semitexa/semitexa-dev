...
final class PhpstanRunner
{
    ...
    private function suggestionFor(string $identifier): string
    {
        return match ($identifier) {
            ...
            'semitexa.noOpMapper'                => 'Give the #[AsMapper] a distinct domainModel: create a dedicated domain (business) model class separate from the persistence resource model, point domainModel: at it, and translate fields explicitly in toDomain() / toSourceModel() instead of cloning the resource model. resourceModel and domainModel must never be the same class.',
            'semitexa.domainModelEncapsulation'  => 'Encapsulate the domain model: make every property `private` and expose it through accessors — a get<Field>() (or is/has) for each field, plus a set<Field>() for mutable fields (mark the property readonly if it is immutable). Public/protected fields on a mapped domain model are forbidden; only ORM resource models keep public promoted properties.',
