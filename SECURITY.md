...
- **GitHub Security Advisories** — [open a private report](https://github.com/semitexa/semitexa-dev/security/advisories/new).
...
In scope: anything in `semitexa/dev` that lets a request, a job or a message do
...
- reports that need the development tools (Explorer, Trace, Quality) switched on in
  a production deployment — they are off outside `dev` mode by design, and a way
  to reach them in production *is* in scope;
